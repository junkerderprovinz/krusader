...
# TITLE feeds the PWA manifest and SELKIES_UI_TITLE is the visible tab and
# sidebar title of the Selkies web client; this base needs both.
#
# Selkies' server turns basic auth on by default with the well-known
# credentials ubuntu/mypasswd, which would put a login with an insecure default
# in front of a container that never set a password. Switching it off keeps the
# KasmVNC behaviour: no login unless CUSTOM_USER/PASSWORD are set. The base's
# nginx would still turn a set but empty PASSWORD into a login, so the
# init-nologin oneshot drops an empty PASSWORD/CUSTOM_USER before nginx starts.
# Selkies binds to localhost only, so with a real CUSTOM_USER/PASSWORD the
# base's nginx enforces basic auth on the proxy, the single reachable entry
# point.
ENV TITLE="Krusader" \
