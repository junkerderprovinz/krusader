#!/usr/bin/env bash
# Behaviour tests for rootfs/usr/local/bin/krusader-session, which supervises
# krusader inside the container. They run the real script against stub
# binaries on PATH, so the actual signal and exit handling is exercised.
#
# Run: tests/test-krusader-session.sh   (or `just test`)
set -uo pipefail

# Run through bash: the shipped scripts are 0644 in git and get their +x from
# the Dockerfile, so a fresh checkout has no executable bit.
...
# 1) Quitting krusader gives the user a new krusader, not a black screen.
...
# 2) Container shutdown ends the session instead of restarting krusader.
sandbox 'sleep 30'
...
# 3) A krusader that cannot start at all does not spin: the session gives up
#    after a few immediate failures instead of burning CPU in a restart loop.
sandbox 'exit 1'
...
# 4) The spin guard also catches a krusader that exits cleanly but instantly
#    every time, such as a second instance handing over to a unique-instance
#    owner that is already gone. Exit code 0 alone does not buy an endless
#    restart loop.
sandbox 'exit 0'
timeout 120 bash "${SESSION}" > "${SB}/out" 2>&1
rc=$?
...
# 5) Shutdown asks krusader to quit itself before resorting to a signal, so its
#    "save settings on exit" gets the chance to run (bug #1).
sandbox 'sleep 30'
